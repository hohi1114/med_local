...
export function extractDistrictFromNeighborhood(
...
/**
 * Parse a polygon string to a nested array structure
 */
export function parsePolygon(polygonStr: string): [number, number][] {
  try {
    // Remove any extra quotes if present
    if (polygonStr.startsWith('"') && polygonStr.endsWith('"')) {
      polygonStr = polygonStr.slice(1, -1);
    }

    // Parse the string to a nested array and flatten if needed
...
export function findMatchingRegion(
  ...
  for (const region of regions) {
    const polygonCoords = flattenToPairs(region.coords);
