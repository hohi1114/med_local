...
/**
 * Extract district (구) name from a neighborhood (동) name
 * Example: "서울특별시 중구 장충동" -> "서울특별시 중구"
 */
...
export function isPointInPolygon(
    ...
    for (let i = 0, j = polygonCoords.length - 1; i < polygonCoords.length; j = i++) {
        ...
        // If it is, toggle the 'inside' flag
        if (intersect) {
            inside = !inside;
        }
    }
 ...
 */
function flattenToPairs(nested: any): [number, number][] {
