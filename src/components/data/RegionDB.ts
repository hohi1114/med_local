...
export async function getPatientsFromRegion(areaName: string) {
...
//The number of tables in the regionDB except for the regionSums and etc
export async function getRegionDBCount() {
  const db = await openDB(REGION_DB_NAME, 1);
  return db.objectStoreNames.length - 2;
}
